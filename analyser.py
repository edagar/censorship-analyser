...
def probeDirectoryAuthorities():
    ...
    if unreachable:
        print "%i of the directory servers (%s) didn't respond the our ping." %
        (len(unreachable), ", ".join([test.target for test in unreachable]))
...
def makeURL(host, port):
    if port == 80:
        ...
        url = "http://%s:%s/tor/status-vote/current/consensus.z" % (
            host, str(port))
    return url
